  ...
  <>
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
      {[
        ...
        <div
          key={label}
          style={{ padding: '8px 10px', background: 'var(--color-paper-surface)', borderRadius: 6 }}
        >
          <div
            style={{
              display: 'flex',
          ...
          >
            ...
            <span style={{ fontSize: 16, fontWeight: 700, color: 'var(--color-paper-dark)' }}>
              {value}
            </span>
          </div>
...
const ThemeCard = ({ node, themeId, raw, onClose }) => {
  const paperIds = (raw?.edges || [])
    ...
    .map((e) => e.source);
  const papers = paperIds.map((id) => raw.nodes.find((n) => n.id === id)).filter(Boolean);
...
const PaperCard = ({ node, onClose }) => (
  <>
    ...
    >
      {node.year && (
        <div>
          <Icon icon="mdi:calendar-outline" style={{ fontSize: 13, marginRight: 5 }} />
          {node.year}
        </div>
      )}
      <div>
        <Icon icon="mdi:format-quote-close" style={{ fontSize: 13, marginRight: 5 }} />
        {node.citation_count ?? 0} citations
      </div>
      {node.source && (
        <div>
          <Icon icon="mdi:database-outline" style={{ fontSize: 13, marginRight: 5 }} />
          {node.source}
        </div>
      )}
    </div>
      ...
      {node.snippet && (
        <blockquote
          style={{
            ...
            margin: '10px 0 0',
            padding: '6px 10px',
            borderLeft: '2px solid var(--color-paper-light)',
            fontStyle: 'italic',
          }}
        >
          "{node.snippet}"
...
const NodeDetailCard = ({ node, nodeId, raw, stats, onClose }) => {
  ...
  return (
    ...
    >
      ...
      {node.type === 'theme' && (
        <ThemeCard node={node} themeId={nodeId} raw={raw} onClose={onClose} />
      )}
