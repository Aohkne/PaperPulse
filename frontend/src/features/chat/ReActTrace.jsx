...
const PHASE_DEFS = [
  ...
  { key: 'analyze', label: 'Analyze & verify',  icon: 'mdi:text-box-check-outline', stepNums: ['4', '5', '6'] },
...
const TYPE_CFG = {
  ...
  observation: { icon: 'mdi:eye-check-outline', label: 'Observation', color: 'var(--color-paper-mid)' },
};
...
const STAT_LABELS = { papers: 'bài báo', themes: 'chủ đề', claims: 'trích dẫn', contradicts: 'mâu thuẫn' };
function finalTally(steps) {
  ...
  if (!raw) return null;
  const pairs = raw.trim().split(/\s+/).map((pair) => pair.split('='));
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        {showTypeLabel && (
          <span style={{
            ...
            color: cfg.color, opacity: 0.75, marginBottom: '2px',
          }}>
            {cfg.label}
          </span>
        )}
        {step.type === 'action' ? (
          <code style={{
            ...
            fontSize: '11px', lineHeight: '1.5', display: 'block', wordBreak: 'break-word',
          ...
          <p style={{
            ...
            color: 'var(--color-paper-mid)', lineHeight: '1.5', margin: 0,
...
const PhaseRow = ({ phase, isLast }) => {
  ...
  return (
    <div style={{ position: 'relative', paddingLeft: '26px' }}>
      {!isLast && (
        <div style={{
          position: 'absolute', left: '9px', top: '22px', bottom: '-2px',
          width: '1px', background: 'var(--color-hairline-border)',
        }} />
      )}
      <button
        ...
        style={{
          ...
          cursor: phase.steps.length > 0 ? 'pointer' : 'default', textAlign: 'left',
        }}
      >
        <div style={{
          position: 'absolute', left: 0, width: '20px', height: '20px', borderRadius: '50%',
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{
            fontFamily: "'Newsreader', serif", fontSize: '13px', fontWeight: 500,
          ...
          {summary && (
            <div style={{
              fontSize: '12px', color: 'var(--color-paper-mid)', marginTop: '1px',
        ...
        {statusIcon && (
          <Icon icon={statusIcon.icon} style={{ fontSize: '15px', color: statusIcon.color, flexShrink: 0, marginTop: '2px' }} />
        )}
        {phase.steps.length > 1 && (
          <Icon
            icon={open ? 'mdi:chevron-up' : 'mdi:chevron-down'}
            style={{ fontSize: '14px', color: 'var(--color-paper-mid)', flexShrink: 0, marginTop: '3px', opacity: 0.6 }}
          />
        )}
      </button>
      ...
      <AnimatePresence initial={false}>
        {open && phase.steps.length > 1 && (
          ...
          >
            <div style={{
              paddingLeft: '10px', marginTop: '2px', marginBottom: '4px',
              borderLeft: '2px solid var(--color-hairline-border)',
            }}>
              {phase.steps.slice(0, -1).map((step) => <NestedStep key={step.id} step={step} />)}
            </div>
          </motion.div>
        )}
      </AnimatePresence>
...
const ReActTrace = ({ steps }) => {
  ...
  const headerIconColor = isRunning ? 'var(--color-brand-500)' : hasError ? '#dc2626' : 'var(--color-paper-mid)';
