...
function stripGapSummary(text) {
  ...
  return text
    .replace(
      /^Showing top .*?research gaps by quality .*?See details in each gap card below\.?\s*$/gim,
      ''
    )
...
const Centered = ({ icon, children }) => (
  <div
    style={{
      ...
      fontFamily: 'var(--font-inknut)',
      background: 'var(--color-paper-surface)',
      border: '1px dashed var(--color-paper-light)',
      borderRadius: '14px',
      padding: '40px 24px',
    }}
  >
...
function GapCard({ gap }) {
  ...
  const hasDetails = Boolean(
    gap.suggested_method || gap.falsifiability_condition || gap.quality_score != null
  );
  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: '0 8px 18px rgba(41, 17, 0, 0.03)',
        fontFamily: 'var(--font-inknut)',
        display: 'flex',
        flexDirection: 'column',
      }}
    >
      ...
      >
        <span
          title={
            gap.gap_type === 'topical'
              ? 'A gap about an underexplored research topic or direction.'
              : undefined
          }
      ...
      <div
        style={{
          marginBottom: '12px',
          padding: '12px 14px',
          borderLeft: '4px solid var(--color-paper-light)',
          borderRadius: '0 12px 12px 0',
          background:
            'linear-gradient(90deg, var(--color-paper-surface) 0%, var(--color-paper-bg) 100%)',
        }}
      >
        <div
          style={{
      ...
      {hasDetails && (
        <div style={{ marginBottom: '10px' }}>
          ...
          >
            <span>
              {detailsOpen ? 'Hide quality & method details' : 'Show quality & method details'}
            </span>
            <Icon
        ...
        <div style={{ borderTop: '1px solid var(--color-paper-surface)', paddingTop: '10px' }}>
          ...
          >
            {gap.supporting_papers.map((p, idx) => {
              ...
              const body = authorStr ? `${authorStr} ${year}. ${title}.` : `${title}. ${year}.`;
...
const GapResultPanel = ({ narrative, gapReport, loading, error }) => {
  ...
  if (error)
    return (
      <Centered icon="mdi:alert-circle-outline">
        {friendlyError(error, "We couldn't analyze research gaps for this topic.")}
      </Centered>
    );
  if (!narrative)
    return (
      <Centered icon="mdi:lightbulb-search-outline">Enter a topic to begin analysis.</Centered>
    );
    ...
    <div style={{ display: 'flex', flexDirection: 'column' }}>
      ...
      <div
        style={{
          ...
          borderRadius: '14px',
          fontFamily: 'var(--font-inknut)',
          boxShadow: '0 10px 28px rgba(41, 17, 0, 0.04)',
        }}
      >
