...
function parseLatexBlocks(raw) {
  ...
  for (const rawLine of body.split('\n')) {
    ...
    if (
      /^\\(documentclass|usepackage)\b/.test(line) ||
      line === '\\begin{document}' ||
      line === '\\end{document}'
    )
      continue;
    if (/^\\begin\{thebibliography\}/.test(line)) {
      blocks.push({ type: 'bibsection_start' });
      continue;
    }
    if (line === '\\end{thebibliography}') {
      blocks.push({ type: 'bibsection_end' });
      continue;
    }
    const bibM = line.match(/^\\bibitem\{([^}]*)\}/);
    if (bibM) {
      blocks.push({ type: 'bibitem', key: bibM[1] });
      continue;
    }
    ...
    m = line.match(/^>\s?(.*)$/);
    if (m) {
      blocks.push({ type: 'mdquote', text: m[1] });
      continue;
    }
    ...
    if (line === '\\end{itemize}') {
      if (listStack[listStack.length - 1] === 'itemize') listStack.pop();
      continue;
    }
    ...
    if (line === '\\begin{quote}') {
      blocks.push({ type: 'quote_start' });
      continue;
    }
    if (line === '\\end{quote}') {
      blocks.push({ type: 'quote_end' });
      continue;
    }
    ...
    m = line.match(/^[-*+]\s+(.*)$/);
    if (m) {
      blocks.push({ type: 'item', text: m[1] });
      continue;
    }
    m = line.match(/^(\d+)\.\s+(.*)$/);
    if (m) {
      blocks.push({ type: 'item_num', num: Number(m[1]), text: m[2] });
      continue;
    }
...
function extractBibliography(blocks) {
  ...
  for (let i = 0; i < blocks.length; i++) {
    ...
    let j = i + 1;
    const textParts = [];
    while (j < blocks.length && blocks[j].type === 'para') {
      textParts.push(blocks[j].text);
      j++;
    }
...
function renderCiteKeys(rawKeys, keyPrefix, bibliography) {
  ...
  return keys.map((citeKey, i) => {
    ...
    if (refIndex === undefined) {
      return (
        <span key={nodeKey} style={citationStyle}>
          ({citeKey})
        </span>
      );
    }
    return (
      <span
        ...
        onKeyDown={(e) => {
          if (e.key === 'Enter' || e.key === ' ') jumpToCitation(citeKey);
        }}
...
function renderInline(text, keyPrefix, bibliography) {
  ...
  for (const m of text.matchAll(INLINE_RE)) {
    ...
    if (g.urlOnly !== undefined) {
      ...
      nodes.push(
        <a key={key} href={g.urlOnly} target="_blank" rel="noopener noreferrer" style={linkStyle}>
          {display}
        </a>
      );
    } else if (g.hrefText !== undefined) {
      nodes.push(
        <a key={key} href={g.hrefUrl} target="_blank" rel="noopener noreferrer" style={linkStyle}>
          {g.hrefText}
        </a>
      );
      ...
      nodes.push(
        <span key={key} style={citationStyle}>
          (Source: {g.sourceCite})
        </span>
      );
      ...
      nodes.push(
        <a key={key} href={g.mdlinkurl} target="_blank" rel="noopener noreferrer" style={linkStyle}>
          {g.mdlinktext}
        </a>
      );
      ...
      nodes.push(
        <code key={key} style={citationStyle}>
          {g.mdcode}
        </code>
      );
    } else if (m[0] === '\\\\') {
...
const LatexPreview = ({ content, emptyText = 'Nothing to preview yet.' }) => {
  ...
  while (i < blocks.length) {
    ...
    if (b.type === 'h1') {
      elements.push(
        <h1 key={key++} style={styles.h1}>
          {renderInline(b.text, `h1-${key}`, bibliography)}
        </h1>
      );
      i++;
      continue;
    ...
    if (b.type === 'h2') {
      elements.push(
        <h2 key={key++} style={styles.h2}>
          {renderInline(b.text, `h2-${key}`, bibliography)}
        </h2>
      );
      i++;
      continue;
    ...
    if (b.type === 'h3') {
      elements.push(
        <h3 key={key++} style={styles.h3}>
          {renderInline(b.text, `h3-${key}`, bibliography)}
        </h3>
      );
      i++;
      continue;
    ...
    if (b.type === 'verbatim') {
      const lines = [];
      while (i < blocks.length && blocks[i].type === 'verbatim') {
        lines.push(blocks[i].text);
        i++;
      }
      elements.push(
        <pre key={key++} style={styles.pre}>
          <code style={styles.code}>{lines.join('\n')}</code>
        </pre>
      );
      continue;
    }
        ...
        <ListTag key={key++} style={styles.list}>
          {items.map((it, idx) => (
            <li key={idx} style={styles.li}>
              {renderInline(it.text, `li-${key}-${idx}`, bibliography)}
            </li>
          ))}
        </ListTag>
      );
    ...
    if (b.type === 'mdquote') {
      const lines = [];
      while (i < blocks.length && blocks[i].type === 'mdquote') {
        lines.push(blocks[i].text);
        i++;
      }
      elements.push(
        <blockquote key={key++} style={styles.blockquote}>
          {renderInline(lines.join(' '), `mdq-${key}`, bibliography)}
        </blockquote>
      );
      continue;
    }
    ...
    if (b.type === 'quote_start') {
      ...
      elements.push(
        <blockquote key={key++} style={styles.blockquote}>
          {renderInline(innerText.join(' '), `bq-${key}`, bibliography)}
        </blockquote>
      );
      continue;
    }
    ...
    if (b.type === 'bibsection_start') {
      ...
      while (i < blocks.length && blocks[i].type !== 'bibsection_end') {
        if (blocks[i].type === 'bibitem') {
          ...
          i++;
          const textParts = [];
          while (i < blocks.length && blocks[i].type === 'para') {
    ...
    if (b.type === 'bibsection_end' || b.type === 'bibitem') {
      i++;
      continue;
    }
    ...
    if (b.type === 'para') {
      ...
      elements.push(
        <p key={key++} style={styles.p}>
          {renderInline(lines.join(' '), `p-${key}`, bibliography)}
        </p>
      );
      continue;
    }
