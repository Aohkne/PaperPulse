...
export function computeRadialLayout(graph) {
  ...
  const papersByTheme = {};
  graph.filterNodes((_, attrs) => attrs.kind === 'paper').forEach((paperId) => {
  ...
  Object.entries(papersByTheme).forEach(([themeId, paperIds]) => {
    ...
    paperIds.forEach((paperId, j) => {
      const offset = paperIds.length > 1
        ? (j / (paperIds.length - 1) - 0.5) * sector
        : 0;
  ...
  const claimsByPaper = {};
  graph.filterNodes((_, attrs) => attrs.kind === 'claim').forEach((claimId) => {
  ...
  Object.entries(claimsByPaper).forEach(([paperId, claimIds]) => {
    ...
    claimIds.forEach((claimId, k) => {
      const offset = claimIds.length > 1
        ? (k / (claimIds.length - 1) - 0.5) * sector
        : 0;
      const angle = baseAngle + offset;
