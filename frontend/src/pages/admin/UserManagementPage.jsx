...
function buildColumns(onBan, onUnban) {
  return [
    { accessorKey: 'email', header: 'Email', size: 220 },
    {
      ...
      size: 140,
      cell: ({ getValue }) =>
        getValue() || <span style={{ color: 'var(--color-admin-muted)' }}>—</span>,
    },
    {
      accessorKey: 'role',
      header: 'Role',
    ...
    },
    {
      ...
      size: 70,
      cell: ({ getValue }) => (
        <span style={{ fontVariantNumeric: 'tabular-nums' }}>{getValue()}</span>
      ),
    },
    {
      id: 'actions',
      ...
      cell: ({ row }) => {
        ...
        return u.is_banned ? (
          <button onClick={() => onUnban(u.id)} style={actionBtnStyle()}>
            Unban
          </button>
        ) : (
          <button onClick={() => onBan(u.id)} style={actionBtnStyle(true)}>
            Ban
          </button>
        );
      },
          ...
          {['', 'admin', 'user'].map((r) => (
            <button
              ...
              style={{
                ...
                border: '1px solid',
                borderColor:
                  roleFilter === r ? 'var(--color-admin-accent)' : 'var(--color-admin-border)',
                background: roleFilter === r ? 'var(--color-admin-accent-bg)' : 'transparent',
                color:
                  roleFilter === r ? 'var(--color-admin-accent-text)' : 'var(--color-admin-mid)',
                fontSize: 12,
