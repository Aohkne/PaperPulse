        ...
        return (
          <div
            ...
            style={{
              ...
              background: isCurrent ? 'var(--color-admin-accent-bg)' : isReached ? 'transparent' : 'transparent',
...
const Blink = () => (
  <span style={{
    display: 'inline-block', width: 2, height: 12, marginLeft: 2,
...
const RawDetails = ({ data }) => (
  <details style={{ marginTop: 6 }}>
    <summary style={{ fontSize: 11, color: 'var(--color-admin-mid)', cursor: 'pointer' }}>Raw event</summary>
    <pre style={{
      margin: '6px 0 0', fontSize: 11, lineHeight: 1.5, whiteSpace: 'pre-wrap', wordBreak: 'break-word',
      background: 'var(--color-admin-bg)', border: '1px solid var(--color-admin-border)',
      borderRadius: 6, padding: 8, color: 'var(--color-admin-muted)',
    }}>
...
const TimelineEntry = ({ entry, isLastStreaming }) => {
  ...
  switch (entry.kind) {
      ...
      return (
        <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
          <div style={{
            ...
            fontSize: 13, color: 'var(--color-admin-text)',
          }}>
            {entry.text}
          </div>
        </div>
    ...
    case 'step_token':
      return shell(`Step ${entry.stepNum} narrator`, 'mdi:cog-outline', 'var(--color-admin-mid)', 'transparent', (
        <p style={{ margin: 0, fontSize: 12, fontStyle: 'italic', color: 'var(--color-admin-mid)', whiteSpace: 'pre-wrap' }}>
          {entry.text}{isLastStreaming && <Blink />}
        </p>
      ));
    ...
    case 'step': {
      const d = entry.data;
      return shell(`Step ${d.stepNum} complete`, 'mdi:check-circle-outline', '#16a34a', 'rgba(22,163,74,0.08)', (
        <>
          <p style={{ margin: 0, fontSize: 13, color: 'var(--color-admin-text)' }}>{d.content}</p>
          {d.stat && <p style={{ margin: '2px 0 0', fontSize: 11, color: 'var(--color-admin-mid)', fontFamily: 'monospace' }}>{d.stat}</p>}
          {d.refined_query && (
          ...
          {d.plan_description && (
            <p style={{ margin: '2px 0 0', fontSize: 12, color: 'var(--color-admin-muted)' }}>{d.plan_description}</p>
          )}
          <RawDetails data={d} />
        </>
      ));
    ...
    case 'greeting':
      return shell('Greeting (intent=greeting)', 'mdi:hand-wave-outline', 'var(--color-admin-mid)', 'transparent', (
        <p style={{ margin: 0, fontSize: 13, color: 'var(--color-admin-text)' }}>{entry.data.content}</p>
      ));

    case 'clarify':
      return shell('Clarify (intent=clarify)', 'mdi:help-circle-outline', '#d97706', 'rgba(217,119,6,0.08)', (
        <ul style={{ margin: 0, paddingLeft: 18, fontSize: 13, color: 'var(--color-admin-text)' }}>
          {(entry.data.questions || []).map((q, i) => <li key={i}>{q}</li>)}
        </ul>
      ));
    ...
    case 'error':
      return shell('Error', 'mdi:alert-circle-outline', '#dc2626', 'rgba(220,38,38,0.08)', (
        <p style={{ margin: 0, fontSize: 13, color: '#dc2626' }}>{entry.data.message}</p>
      ));

    case 'resume_action':
      return shell('Resumed', 'mdi:play-circle-outline', 'var(--color-admin-accent)', 'var(--color-admin-accent-bg)', (
        <p style={{ margin: 0, fontSize: 12, color: 'var(--color-admin-text)', fontFamily: 'monospace' }}>
          resume_value = {JSON.stringify(entry.data.resumeValue)}
        </p>
      ));

    default:
      return shell(entry.kind, 'mdi:code-json', 'var(--color-admin-mid)', 'transparent', (
        <RawDetails data={entry.data} />
      ));
  }
};
  ...
  return (
    <div style={{
      ...
      <pre style={{
        ...
        background: 'var(--color-admin-bg)', border: '1px solid var(--color-admin-border)',
        borderRadius: 6, padding: 8, color: 'var(--color-admin-muted)', whiteSpace: 'pre-wrap',
      }}>
        {JSON.stringify(data, null, 2)}
      </pre>
      <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
        <button
          ...
          style={{
            ...
            fontSize: 13, fontWeight: 700, background: '#d97706', color: '#fff',
        ...
        <input
          ...
          style={{
            flex: 1, minWidth: 220, padding: '7px 10px', fontSize: 12, fontFamily: 'monospace',
        ...
        <button
          ...
          style={{
            ...
            border: '1px solid var(--color-admin-border)', borderRadius: 6,
            background: 'transparent', color: 'var(--color-admin-text)', cursor: 'pointer',
          }}
        >
          Resume with custom value
        </button>
      </div>
      {customError && <p style={{ margin: '6px 0 0', fontSize: 11, color: '#dc2626' }}>{customError}</p>}
    </div>
  );
};
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16, height: '100%' }}>
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', gap: 8 }}>
        <input
          ...
          style={{
            ...
            border: '1px solid var(--color-admin-border)', borderRadius: 8,
            background: 'var(--color-admin-input-bg)', color: 'var(--color-admin-text)',
          }}
        />
        <button
        ...
        >
          {running ? <Icon icon="mdi:loading" className="animate-spin" style={{ fontSize: 15 }} /> : <Icon icon="mdi:play" style={{ fontSize: 15 }} />}
        ...
        <button
          ...
          style={{
            ...
            color: 'var(--color-admin-text)', cursor: running ? 'default' : 'pointer',
          }}
        >
      ...
      {error && (
        <div style={{
          ...
          background: 'rgba(220,38,38,0.08)', border: '1px solid rgba(220,38,38,0.3)', borderRadius: 8,
        }}>
          {error}
        </div>
      )}
      ...
      {/* Timeline */}
      <div style={{
        flex: 1, minHeight: 0, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 8,
        padding: 4,
      }}>
        ...
        <AnimatePresence initial={false}>
          {timeline.map((entry) => (
            ...
            >
              <TimelineEntry entry={entry} isLastStreaming={running && entry.id === lastEntry?.id} />
            </motion.div>
          ))}
        </AnimatePresence>
        <div ref={bottomRef} />
