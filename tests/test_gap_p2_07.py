...
async def test_extractor_persist_specter2_success():
    ...
    with patch("backend.agent.gap_detection.nodes.extractor.get_paper_detail", new_callable=AsyncMock) as mock_detail, \
         ...
         mock_extract.return_value = ExtractedPaperData(paper_ref=paper_ref, abstract="abs", key_claims=["C1"], extraction_source="abstract")
...
async def test_build_background_corpus_success():
    ...
    with patch("backend.agent.gap_detection.retrieval.search", new_callable=AsyncMock) as mock_search, \
         ...
         patch("backend.agent.gap_detection.background_corpus.get_embeddings_batch", new_callable=AsyncMock) as mock_embed, \
...
async def test_orchestrator_fire_and_forget():
    ...
    with patch("backend.agent.gap_detection.orchestrator.is_query_analyzer_enabled", return_value=False), \
         ...
         patch("backend.agent.gap_detection.orchestrator.build_background_corpus", new_callable=AsyncMock) as mock_build_bg:
