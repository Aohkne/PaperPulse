...
def test_gapitem_backward_compat():
    """Scenario: GapItem cũ không có novelty/false_gap fields"""
    data = {
        "statement": "Test gap",
        "gap_type": "topical",
        "origin": "limitation",
...
# Feature: false_gap dùng nim_store

@pytest.mark.asyncio
async def test_check_false_gap_flagged():
    """Scenario: gap_statement gần với corpus (distance < 0.15) → flagged"""
...
async def test_check_false_gap_not_flagged():
    """Scenario: gap_statement xa corpus → không flagged"""
    with patch("backend.agent.gap_detection.false_gap.embed_text", new_callable=AsyncMock) as mock_embed, \
         patch("backend.agent.gap_detection.false_gap.query_with_distances_nim") as mock_query, \
         patch("backend.agent.gap_detection.false_gap.get_false_gap_threshold") as mock_threshold:
...
async def test_check_false_gap_empty_or_fail():
    """Scenario: nim_store rỗng / embed_text fail → False (fail safe)"""
    with patch("backend.agent.gap_detection.false_gap.embed_text", new_callable=AsyncMock) as mock_embed:
        mock_embed.side_effect = Exception("API fail")
        res = await check_false_gap("Test")
        assert res is False

# Feature: novelty dùng nim_store
...
async def test_background_corpus_dual_upsert():
    ...
    with patch("backend.agent.gap_detection.retrieval.search", new_callable=AsyncMock) as mock_search, \
         ...
         patch("backend.agent.gap_detection.background_corpus.get_embeddings_batch", new_callable=AsyncMock) as mock_s2_embed, \
...
# Feature: extractor upsert dual store
@pytest.mark.asyncio
