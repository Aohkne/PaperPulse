...
Implements the 6-step cold-start flow (TIP-G04, updated G11):
  ...
  1. clean_query       - VN->EN, strip meta-words            (query_cleaner.py)
  2. search            - Semantic Scholar keyword search     (retrieval.py)
  2b. fallback         - retry with raw topic if search < 3  (G11.2, in-zone)
  3. snowball          - citation graph expansion            (retrieval.py)
  4. rank              - embedding-free deterministic top-K  (retrieval.py)
  5. gate              - insufficient-paper early return
  6. run_gap_detection - full LangGraph pipeline             (graph.py)
...
from backend.module.gap_detection.background_corpus import build_background_corpus
from backend.module.gap_detection.graph import run_gap_detection
from backend.module.gap_detection.nodes.coherence_check import check_coherence
from backend.module.gap_detection.nodes.query_analyzer import analyze_query
from backend.module.gap_detection.nodes.relevance_gate import run_relevance_gate
from backend.module.gap_detection.query_cleaner import clean_query
from backend.module.gap_detection.schemas import GapQuery, GapReport, PaperRef, QueryRejectedError
...
async def cold_start(topic: str) -> GapReport:
    ...
    Returns:
        A :class:`~backend.module.gap_detection.schemas.GapReport` that is
    ...
    if is_query_analyzer_enabled():
        ...
        try:
            ...
            logger.info(
                "cold_start: Stage A GapQuery: core=%r facets=%s valid=%s reason=%s",
                gap_query.core_topic,
                gap_query.facets,
                gap_query.is_research_topic,
                gap_query.reject_reason,
            )
            if not gap_query.is_research_topic:
                reason = gap_query.reject_reason or "off_topic"
                logger.info("cold_start: Stage A guardrail rejected query - reason=%s", reason)
                raise QueryRejectedError(reason)
    ...
    # Step 1: Translate / normalise topic -> English search query
    clean = await clean_query(topic)
    logger.info("cold_start: topic=%r -> clean=%r", topic[:80], clean[:80])

    if cold_start_papers:
        # Stages B+C+D produced a curated paper set - skip search/snowball/rank.
        ...
        # Step 2: Keyword search (up to 100 candidate papers)
        pool = await retrieval.search(clean, limit=100)
        logger.info("cold_start: search returned %d papers", len(pool))

        # Step 2b: Fallback if clean_query over-distilled and search is too thin,
        ...
        search_fallback_threshold = 3
        if len(pool) < search_fallback_threshold and clean.lower() != topic.lower():
            logger.info(
        ...
        # Step 3: Snowball - expand via citation graph (depth=1)
        merged = await retrieval.snowball(pool)
        logger.info("cold_start: after snowball: %d papers", len(merged))

        # Step 4: Rank and take top-K (MAX_PAPERS_FOR_GAP, cap-final=20)
        top_k = get_max_papers_for_gap()
    ...
    # Step 5: Insufficient-paper gate
    ...
    # Step 6: Map Paper -> PaperRef and invoke the full LangGraph pipeline
...
def _papers_to_refs(papers: list) -> list[PaperRef]:
    ...
    for paper in papers:
        ...
        if not paper_id:
            ...
            continue
        ext_ids: dict = getattr(paper, "external_ids", None) or {}
        refs.append(
            PaperRef(
                paper_id=paper_id,
                title=title,
