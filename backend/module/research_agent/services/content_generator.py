...
async def generate_theme_content(
    theme: Theme, top_k: int = 10, papers: list[Paper] | None = None
    ...
    if papers is not None:
        corpus = [{"paperId": p.paper_id, "title": p.title or "", "abstract": p.abstract or ""} for p in papers]
        paper_map = {p.paper_id: p for p in papers}
    else:
        all_meta = await get_all_papers_metadata()
        corpus = [
            {"paperId": m["paperId"], "title": m.get("title", ""), "abstract": m.get("abstract", "")} for m in all_meta
        ]
        paper_map = {}
